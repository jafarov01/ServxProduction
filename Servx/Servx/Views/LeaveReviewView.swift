...
struct LeaveReviewView: View {
    ...
    init(bookingId: Int64, providerName: String, serviceName: String) {
        _viewModel = StateObject(wrappedValue: LeaveReviewViewModel(
            bookingId: bookingId,
        ...
        NavigationView {
            ScrollView {
                VStack(alignment: .leading, spacing: 20) {
                    ...
                    // Context Text
                    Text("Leave a review for \(viewModel.providerName)'s \(viewModel.serviceName) service")
                        .font(.headline)
                        .padding(.bottom, 10)
                    ...
                    Button {
                        ...
                        HStack {
                            ...
                            Text(viewModel.isLoading ? "Submitting..." : "Submit Review")
                        }
                        .frame(maxWidth: .infinity)
                        .frame(height: 44)
                    }
    ...
    private func starRatingInput(rating: Binding<Double>) -> some View {
        HStack(spacing: 8) {
            ForEach(1...5, id: \.self) { number in
                Image(systemName: number > Int(rating.wrappedValue.rounded(.up)) ? "star" : "star.fill")
                    .resizable()
                    .scaledToFit()
                    .frame(width: 35, height: 35)
                    .foregroundColor(number > Int(rating.wrappedValue.rounded(.up)) ? Color.gray.opacity(0.5) : Color.yellow)
                    .onTapGesture {
