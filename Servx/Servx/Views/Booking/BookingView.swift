...
struct BookingView: View {
    ...
    @StateObject private var viewModel = BookingViewModel(
        // Assuming AuthenticatedUser.shared provides the necessary user info
        authenticatedUser: AuthenticatedUser.shared
        // Note: If BookingViewModel needs NavigationManager, it should be passed here
        // or fetched from the environment within the ViewModel if appropriate.
        // Based on the user's choice, it's fetched from Environment below.
    )
                    ...
                    ForEach(viewModel.bookings) { booking in
                        BookingCardView(booking: booking, viewModel: viewModel)
                            ...
                            .onAppear {
                                ...
                                let canLoad = viewModel.canLoadMore[viewModel.selectedTab] ?? false
                                if booking.id == viewModel.bookings.last?.id && canLoad && !viewModel.isLoading {
                                    Task {
    ...
    private func handleChatNavigation(targetId: Int64?) {
        ...
        print("BookingView: Reacting to chatNavigationTarget change, navigating to \(requestId)")
                ...
                Button {
                    ...
                    VStack(spacing: 4) {
                        Text(tab.rawValue)
                            ...
                            .foregroundColor(selectedTab == tab ? ServxTheme.primaryColor : ServxTheme.secondaryTextColor)

                        Rectangle()
                            .frame(height: 3)
                            .foregroundColor(selectedTab == tab ? ServxTheme.primaryColor : .clear)
                    }
...
struct BookingCardDetailsView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 12) { // Use consistent spacing
            ...
            detailRow(label: "Date:", value: booking.scheduledStartDate?.formatted(date: .long, time: .omitted) ?? "-")
            detailRow(label: "Time:", value: booking.scheduledStartDate?.formatted(date: .omitted, time: .shortened) ?? "-")
            detailRow(label: "Est. Duration:", value: "\(booking.durationMinutes) minutes")
            detailRow(label: "Est. Price:", value: formattedPriceRange(min: booking.priceMin, max: booking.priceMax))
            ...
            if bookingCoordinate != nil {
                 ...
                 Button {
                    if let coordinate = bookingCoordinate {
                         openAppleMapsForDirections(coordinate: coordinate, name: booking.serviceName)
                    }
                 } label: {
    ...
    private func geocodeAddressAndSetupMap() async {
        print("BookingCardDetailsView: Attempting to geocode address: \(fullAddress)")
        let geocoder = CLGeocoder()
        do {
            // Attempt geocoding
            let placemarks = try await geocoder.geocodeAddressString(fullAddress)
            if let coordinate = placemarks.first?.location?.coordinate {
                print("BookingCardDetailsView: Geocoding successful - Lat: \(coordinate.latitude), Lon: \(coordinate.longitude)")
                ...
                self.mapCameraPosition = .region(MKCoordinateRegion(
                    center: coordinate,
                    span: MKCoordinateSpan(latitudeDelta: 0.02, longitudeDelta: 0.02) // Adjust zoom
                ))
            } else {
                 print("BookingCardDetailsView: Geocoding returned no location for address.")
                 self.bookingCoordinate = nil // Ensure it's nil if geocoding fails
            }
        } catch {
            print("BookingCardDetailsView: Geocoding failed for address '\(fullAddress)': \(error)")
            self.bookingCoordinate = nil // Ensure it's nil on error
        }
    }

    private func openAppleMapsForDirections(coordinate: CLLocationCoordinate2D, name: String?) {
...
struct BookingCardView: View {
    ...
    // Helper to get the relevant participant info (seeker or provider)
    private var otherParticipant: (name: String, photoUrl: String?) {
        viewModel.getOtherParticipant(for: booking)
    }
    
    // Helper to get the relevant participant's image URL for the card
    // Depending on who is viewing, show the other person's picture
     private var participantImageURL: URL? {
         URL(string: otherParticipant.photoUrl ?? "")
     }
            ...
            HStack(alignment: .top, spacing: 12) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    HStack(spacing: 4) {
                        ...
                        Text(booking.scheduledStartDate?.formatted(date: .abbreviated, time: .omitted) ?? "No Date")
                    }
                    .font(.caption)
                    .foregroundColor(ServxTheme.secondaryTextColor)
                    ...
                    HStack(spacing: 4) {
                         ...
                         Text(booking.scheduledStartDate?.formatted(date: .omitted, time: .shortened) ?? "No Time")
                    }
                    .font(.caption)
                    .foregroundColor(ServxTheme.secondaryTextColor)
                }
